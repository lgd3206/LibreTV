...
class StatsUI {
    ...
    createStatsPanel() {
        ...
        panel.innerHTML = `
            <div class="stats-header">
                <h3>📊 网站统计</h3>
                <button class="stats-toggle" onclick="statsUI.togglePanel()">×</button>
            </div>
            <div class="stats-content">
                <div class="stats-grid">
                    ...
                    <div class="stat-item">
                        <div class="stat-number" id="today-views">-</div>
    ...
    addStyles() {
        ...
        styles.textContent = `
            .stats-panel {
                ...
                transform: translateX(${this.isVisible ? '0' : '300px'});
    ...
    init() {
        ...
        // 创建统计面板
        this.container = this.createStatsPanel();
    ...
    async updateStats() {
        try {
            ...
            if (stats) {
                ...
                // 更新热门页面
                const pagesList = document.getElementById('popular-pages-list');
                if (stats.popularPages && stats.popularPages.length > 0) {
                    pagesList.innerHTML = stats.popularPages
                        .slice(0, 5)
                        .map(item => `
                            <div class="page-item">
                                <span class="page-path" title="${item.page}">${item.page}</span>
                                <span class="page-count">${item.count}</span>
                            </div>
                        `).join('');
                } else {
                    pagesList.innerHTML = '<div class="page-item">暂无数据</div>';
                }
